...
判据：传入的 ``audience`` 文本命中中 / 英 / 越三语的儿童线索词，或命中形如 ``6-10 岁``、
``6–10 tuổi``、``ages 6-10``、``6 to 10 years old``、``6-10 year-olds`` 这类**带年龄标记词**
的年龄区间，即判定为儿童受众、交回儿童 gear 规则块；否则返回空串，调用方不渲染任何受众
专属规则——成人 / 未设受众的 prompt 与加这个功能之前逐字相同。

年龄区间必须**紧邻年龄标记词**（岁 / 歲 / tuổi / years / year-old(s) / y/o / age(s)）才算数：
``resolve_project_audience_text`` 会退回 overview 的 world_setting / theme 文本兜底，那些字段
常出现「第 3-9 章」「tập 6-10」「chapters 5-8」这类与年龄无关的数字区间（集数、章节号），
裸区间（不带标记词）不触发儿童 gear，避免把成人向小说误判成儿童内容。
...
#: 年龄区间线索：数字区间（分隔符收 ``-`` / ``–`` / ``~`` / ``to`` / 越南语「đến」，数字限制在
#: 3-14，真正的儿童内容年龄区间集中在这一段）**必须紧邻年龄标记词**才算命中——标记词可以在
#: 区间前（「ages 6-10」「age: 6-10」）或区间后（「6-10 岁」「6–10 tuổi」「6 to 10 years old」
#: 「6-10 year-olds」）。裸区间（如「3-9 章」「6-10 集」「chapters 5-8」）不带标记词，不触发。
_KIDS_AGE_RANGE_PATTERN = re.compile(
    r"""
    (?:age|ages)\s*[:\s]*                                              # 前置标记：ages 6-10 / age: 6-10
    (?<!\d)(?:[3-9]|1[0-4])\s*(?:-|–|~|to|đến)\s*(?:[3-9]|1[0-4])(?!\d)
    |
    (?<!\d)(?:[3-9]|1[0-4])\s*(?:-|–|~|to|đến)\s*(?:[3-9]|1[0-4])(?!\d) # 区间在前，后接标记词
    \s*(?:岁|歲|tuổi|y\s*/\s*o\b|years?[\s-]*old[s]?|years?\b)
    """,
