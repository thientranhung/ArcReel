...
# 开场承接要求：与 drama script_plan 同口径——第二集起首个 unit 的口播须承接上集预告 / 钩子。
_OPENING_BRIDGE_GUIDE = (
    "开场（第一个 unit）须用一两句画外音承接上集：呼应上集的下集预告语或集尾钩子、交代时间过渡，"
    "再自然进入本集原文；只点到为止，不要复述上集剧情，也不要把上集内容当作本集的新 unit 重新展开。"
)
